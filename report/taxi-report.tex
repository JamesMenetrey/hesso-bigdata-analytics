...
This solution has been implemented in C\#, in the .NET Core application called \emph{DataWalker}. The source code can be found in the \texttt{src} repertory on Github. Spark has been used to preprocess the dataset and export the clean data into multiple CSV files. The application then reads the files and creates new files with the duplicated records. Finally, the new files are imported in Spark to run the machine learning algorithms.
...
Another C\# application called \emph{GeoJsonParser} has been written to convert the centroids coordinates generated by Spark into the format GeoJSON supported by the interactive map component \emph{Mapbox}. The source code can be found in the \texttt{src} repertory on Github.
